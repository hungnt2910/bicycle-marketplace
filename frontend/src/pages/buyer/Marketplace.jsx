...
const Marketplace = ({ onNavigate }) => {
    ...
    const getFilteredBikes = () => {
        ...
        if (filters.condition) {
            ...
            filtered = filtered.filter(bike =>
                bike.condition === conditionMap[filters.condition]
            );
        }
        ...
        <div className="section">
            <div className="container-custom">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-neutral-600">Khám phá hàng ngàn xe đạp chất lượng từ người bán uy tín</p>
                </div>
                        ...
                        <Card className="p-6 sticky top-24">
                            ...
                            <div className="space-y-4">
                                ...
                                <Button
                                    variant="outline"
                                    className="w-full"
                                    onClick={() => setFilters({ type: '', priceRange: '', brand: '', condition: '', verified: false })}
                                >
                                    Xóa bộ lọc
                                </Button>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex gap-1 border border-neutral-300 rounded-lg p-1">
                                    <button
                                        ...
                                        className={`px-3 py-2 rounded text-sm font-medium ${viewMode === 'grid' ? 'bg-primary-100 text-primary-600' : 'text-neutral-600 hover:bg-neutral-100'}`}
                                    >
                                        Grid
                                    </button>
                                    <button
                                        onClick={() => setViewMode('list')}
                                        className={`px-3 py-2 rounded text-sm font-medium ${viewMode === 'list' ? 'bg-primary-100 text-primary-600' : 'text-neutral-600 hover:bg-neutral-100'}`}
                                    >
                                        List
                                    </button>
                                </div>
                        ...
                        <div className={viewMode === 'grid' ? 'grid md:grid-cols-2 xl:grid-cols-3 gap-6' : 'space-y-4'}>
                            {currentBikes.length > 0 ? (
                                currentBikes.map((bike) => (
                                    viewMode === 'grid' ? (
                                        <Card key={bike.id} variant="product" className="overflow-hidden group cursor-pointer" onClick={() => handleProductClick(bike.id)}>
                                            <div className="relative aspect-product bg-neutral-100">
                                                ...
                                                <button className="absolute top-3 left-3 px-3 py-1 bg-white/90 rounded-lg hover:bg-white transition-colors text-xs font-medium">
                                                    Wishlist
                                                </button>
                                            </div>
                                            <div className="p-4">
                                                ...
                                                <div className="flex items-baseline gap-2 mb-3">
                                                    <span className="price text-xl">{bike.price.toLocaleString('vi-VN')} ₫</span>
                                                    {bike.oldPrice && (
                                                        <span className="price-old text-sm">{bike.oldPrice.toLocaleString('vi-VN')} ₫</span>
                                                    )}
                                                </div>
                                        ...
                                        <Card key={bike.id} className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer" onClick={() => handleProductClick(bike.id)}>
                                            <div className="flex gap-4 p-4">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex items-center gap-3 mb-3">
                                                        <Rating value={bike.rating} size="sm" readonly />
                                                        <span className="text-sm text-neutral-600">({bike.reviews} đánh giá)</span>
                                                        <Badge variant="success">{bike.condition}</Badge>
                                                    </div>
                                                    <div className="flex items-baseline gap-2 mb-3">
