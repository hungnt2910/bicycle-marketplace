  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        try {
          const txDetail = await transactionApi.getById(resolvedTxId);
        ...
        if (
          ...
          if (pendingAction === 'inspection' && storedBicycleId && !inspectionRequested && !alreadySent) {
            ...
            try {
              ...
              setMessage('Thanh toán thành công! Yêu cầu kiểm định đã được gửi. Inspector sẽ liên hệ sớm.');
              ...
              setMessage('Thanh toán thành công nhưng không thể gửi yêu cầu kiểm định. Vui lòng liên hệ hỗ trợ.');
              toast.warning('Thanh toán thành công nhưng cần liên hệ hỗ trợ để gửi yêu cầu kiểm định.');
            }
          } else if (txType === 'fee') {
            setMessage('Thanh toán phí đăng bài thành công! Tin đăng của bạn đang chờ admin duyệt.');
          ...
          if (txType === 'fee') {
            setMessage('Thanh toán phí đăng bài thất bại. Bạn có thể thử lại từ trang Quản lý tin đăng.');
          } else if (txType === 'inspection_fee' || pendingAction === 'inspection') {
            setMessage('Thanh toán phí kiểm định thất bại. Bạn có thể thử lại từ trang Quản lý tin đăng.');
          } else {
      ...
      <Card className="w-full max-w-xl p-8 shadow-lg border border-neutral-200/70">
          ...
          <div>
            ...
            {transactionType && (
              <p className="text-neutral-500 text-xs mt-1">
                Loại: {transactionType === 'fee' ? 'Phí đăng bài' : transactionType === 'inspection_fee' ? 'Phí kiểm định' : 'Mua hàng'}
              </p>
            )}
          </div>
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          <Button onClick={handleBackButton}>
            {transactionType === 'fee' || transactionType === 'inspection_fee' ? 'Về trang quản lý tin' : 'Về trang đơn hàng'}
          </Button>
